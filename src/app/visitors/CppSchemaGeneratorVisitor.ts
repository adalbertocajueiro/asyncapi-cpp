...
export class CppSchemaGeneratorVisitor {
    ...
    visitEnum(enumObj: EnumType): string {
        ...
        for (let i = 0; i < enumObj.items.length; i++) {
            ...
            result = result.concat(this.getSpaces(identLevel))
    ...
    buildEqualityOperator(classObj: ClassType, indentLevel: number) {
        ...
        result = result.concat(this.newLine())
        indentLevel = indentLevel + 2
        result = result.concat(this.indent(indentLevel) + 'return')
        result = result.concat(this.newLine())
        indentLevel = indentLevel + 2
        ...
        for (const [index, att] of classObj.attributes.entries()) {
            ...
            if (att.attType == 'string') {
                result = result.concat(this.indent(indentLevel) + 'strcmp(this->' + att.attName + '.c_str()' + ', ' + 'other.' + att.attName + '.c_str()) == 0')
            } else {
                result = result.concat(this.indent(indentLevel) + 'this->' + att.attName + ' == other.' + att.attName)
            }
    ...
    buildToJsonOperation(classObj: ClassType, indentLevel: number) {
        ...
        result = result.concat(this.newLine() + this.indent(indentLevel) + '{')
        indentLevel = indentLevel + 2
        result = result.concat(this.newLine() + this.indent(indentLevel) + 'json result;')
        result = result.concat(this.newLine())
        for (const [index, att] of classObj.attributes.entries()) {
            if (att.attType == 'array') {
                ...
                result = result.concat(this.indent(indentLevel) + tempVariable)
                result = result.concat(this.newLine())
                result = result.concat(this.indent(indentLevel) + 'if(' + att.attName + '.size() > 0)')
                result = result.concat(this.newLine() + this.indent(indentLevel) + '{')
                indentLevel = indentLevel + 2
                result = result.concat(this.newLine() + this.indent(indentLevel))
        ...
        result = result.concat(this.indent(indentLevel) + 'return result;')
        indentLevel = indentLevel - 2
        result = result.concat(this.newLine() + this.indent(indentLevel) + '}')
    ...
    */
    ...
    buildFromJsonOperation(classObj: ClassType, indentLevel: number) {
        ...
        result = result.concat(this.newLine() + this.indent(indentLevel) + '{')
        indentLevel = indentLevel + 2
        result = result.concat(this.newLine() + this.indent(indentLevel) + 'return from_json_string(json_obj.dump());')
        indentLevel = indentLevel - 2
        result = result.concat(this.newLine() + this.indent(indentLevel) + '}')
    ...
    */
    ...
    buildFromJsonStringOperation(classObj: ClassType, indentLevel: number) {
        ...
        result = result.concat(this.newLine() + this.indent(indentLevel) + '{')
        indentLevel = indentLevel + 2
        result = result.concat(this.newLine() + this.indent(indentLevel) + 'json json_obj = json::parse(json_string);')
        result = result.concat(this.newLine() + this.newLine())

        result = result.concat(this.indent(indentLevel) + classObj.className + ' result = ' + classObj.className + '();')
        result = result.concat(this.newLine())
        ...
        for (const [index, att] of classObj.attributes.entries()) {
            ...
            if (att.attType == 'array') {
                ...
                result = result.concat(this.indent(indentLevel) + tempVariableDecl)
                result = result.concat(this.newLine() + this.indent(indentLevel))
        ...
        result = result.concat(this.newLine() + this.indent(indentLevel) + 'return result;')
